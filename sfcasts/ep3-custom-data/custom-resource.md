...
Ultimately we're still querying for and saving to a specific entity like jeez listing
and user, but really an API resource can be any object that pulls data from anywhere.
So instead of adding an unprecedented is me and his MVP fields to user, we could have
...
API resource. So think of an end point like /API /daily stats that returns a
collection of stats where each stat contains sites statistics for a single day. Now
we're not going to create a daily stats entity because I want to instead pretend like
...
directory create a new class called daily stats. Now notice I put this in the entity,
even though this is not going to be a doctrine entity, and that is up to you. The
only thing you need to be aware of is that in config packages, API platform,
diameter, there's a little bit config up here that tells API platform to look in the
NC directory for classes that have the API resource annotation. So if you're gonna,
...
So I'll say public, and we're going to add three properties, date, public, total
visitors. Let's also do a public most popular listings. So that would actually, uh,
contain jeez listing objects that were the most popular for that day. And then above
those to make this an APR resource, we will add at API resource and it leads to
...
dash stats. So we can actually fix that by going in and changing these short name =
daily dash stats.
...
it that converts a daily stats to daily_stats or daily dash stats. This is called the
path segment name generator, and it's actually something you can customize. And by
...
get rid of the daily stats and an API platform. It doesn't matter where I will paste
that. So this now says to use this other path segment name generator, which uses
...
I am operations. He goes empty and then collection operations equals. And we'll just
...
/daily dash stats that JSON LD and it sort of works, but it's empty in reality, this
...
So inside the data provider directory, I'm going to create a new class called a daily
stats provider. And we'll keep this as simple as possible. We need to implement
collection data provider interface, cause we're only going to be providing that
collection end point at least right now, and then the restricted data provider
interface so that we can say that we just support the daily stats class. So I'll go
to code generate or command and on the Mac, I'm going to "Implement Methods" to
implement the two methods that we need now for supports. It's pretty easy. Return
resource class = = = daily stats, ::class. Now forget collection. Let's just kind of
return a dummy object right now. So we'll say stats = new daily stats, stats,->date =
new /date, time stats, arrow. Total visitors = 100 and we'll just leave the, uh, most
popular listings to be empty right now.
...
that when you use JSON LD, it always generates an ad ID with the I R I to this, uh,
...
So an easiest solution to this would be to actually allow you to have a good item
operation here. And we actually are going to do that later, but let's pretend like we
...
that. Not found action right there. So I'll say use not found action.
...
operation, but not really actually have a good operation. So now let's try it again.
...
an ID or a U U ID, which we'll talk about later in this tutorial. So that means that
...
situation, it's still easy enough. You can say at API property, and then you can say,
identifier = true. Now APAC platform knows this is going to be your identifier. But
...
use this as a string identifier for it. This is a daytime object. So that doesn't
work. It's actually totally okay. We can do a really clever thing here, create a new
public function, get date string, I'll have this return a string.

And then I'll return this->date->format, we'll say Y dash M dash D. And then we're
going to take this API property thing and actually put it above there. So we're sort
of creating is kind of like an, a, another custom property via a Gitter. And it's
...
So what that means is we'll go up here and add normalization contexts = groups = open
curly, close curly, and I'll call this daily dash stats, colon read, and I'll copy
that group name because we need to put it on all the properties that we want. So
we'll put this above date at groups only need one at symbol curly, curly, and I'll
paste.
...
Gitter methods, uh, PHP 7.4, um, types like this and several other spots, which is
...
So one way, in addition to maybe adding at VAR or something above these of the
...
I'm gonna say public function,_underscore construct, and we will have looks at a
daytime interface called date int of total visitors.
...
easy way to do it. And then the only thing we need to fill in is date, time interface
type in, into type. And then we know that the most popular lessons are going to be an
array and I'm actually going to remove most of the documentation. These first two are
redundant. And the only thing about this most popular listening is is that we know
that this is not just going to be an array. It's going to be an array of cheese
listing. So I'll kind of help it out here by saying she's listing less for a bracket,
...
which will be an array of I R I strings, which is awesome.
