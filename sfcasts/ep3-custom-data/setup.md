...
Anyways, one of the steps in the REDME is to find a terminal, move into
the project and run:
...
is a new library that makes it *really* easy to create dummy data. We're
using Foundry inside of our data fixtures - so `src/DataFixtures/AppFixtures.php` -
as a quick way to create a bunch of users and cheese listings. We also use this
inside our test suite. You don't need to know too much about Foundry, but
...
basic functional tests for our 2 API resources: `CheeseListing` and `User`.
...
`database` and `database_test`. Thanks to this, the symfony binary exposes a
...
things. In `.env.test`, I *am* setting `DATABASE_TEST_URL` to `DATABASE_URL`... which
